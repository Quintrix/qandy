...
// ── 1. Parse the item file path ───────────────────────────────────────────
  ...
  function getRegValue(valStr) {
    ...
    if (valStr.charAt(0) === 'W') {
      return parseInt(register[valStr] || "0", 10);
    }
    ...
    switch (word) {
      ...
      default: {
        ...
        if (word.charAt(0) === 'W') {
          // read logic
          var target = word;          
          var action = tape.slice(column, column + 2); column += 2;
          var value  = tape.slice(column, column + 2); column += 2; 
          
          var targetvalue  = parseInt(register[target] || "0", 10);
          var value = getRegValue(value);

          // Execute the Operator
          switch (action) {
            case '==': // SET
              register[target] = value.toString().padStart(2, '0');
              break;

            case '++': // ADD
              var added = targetvalue + value;
              if (added > 99) added = 99; // hard limit
              register[target] = added.toString().padStart(2, '0');
              break;

            case '--': // SUBTRACT
              var subbed = targetvalue - value;
              if (subbed < 0) subbed = 0; // hard limit
              register[target] = subbed.toString().padStart(2, '0');
              break;

            case '??': // DICE ROLL
              var minVal = targetvalue;
              var maxVal = value;
            ...
            case '><': // EQUALS
              ...
              if (!conditionMet) switchopen = true; // Open the switch!
              break;

            case '>>': // GREATER THAN
              conditionMet = (targetvalue > value);
              if (ifnot) conditionMet = !conditionMet;
              if (!conditionMet) switchopen = true;
              break;
            ...
            default:
              console.log('UNIVAC: unknown register operator [' + op + '] at column ' + (column - (op === '??' ? 8 : 6)));
              break;
          }
