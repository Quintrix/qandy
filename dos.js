 ...
 *
 * Local* functions (hardcoded to localStorage, DEVICE-independent):
 *  localLoad(file)     — load file from localStorage
 *  localSave(file, text) — save text to localStorage (">file" appends)
 *  localDelete(file)   — delete file from localStorage (rejects on error)
 *  localExists(file)   — returns boolean (resolves, never rejects)
 *  localDir()          — all entries in manifest format (name|size|timestamp)
 *  localList()         — visible (non-hidden) filenames, newline-separated
 *  localRename(file, dest) — rename file in localStorage (rejects on error)
 *
 * Global variables:
...
(function (global) {
  ...
  global.dosDelete = async function (file) {
    var dev = global.DEVICE; var fname = _normName(file);
    if (_classify(fname) !== 'normal') return 'Error: read only file';
    var fbase = _baseName(fname);
    var fv = _validateBase(fbase);
    if (!fv.ok) return 'Error: ' + fv.reason;
  ...
  // localDelete(file) — delete file from localStorage
  ...
  global.localRename = async function (file, dest) {
    ...
    if (_classify(fname) !== 'normal') throw new Error('read only file');
    if (_classify(dname) !== 'normal') throw new Error('read only file');
    ...
    var dbase = _baseName(dname);
    var dv = _validateBase(dbase);
    if (!dv.ok) throw new Error('invalid destination: ' + dv.reason);

    var manifest = _loadManifest();
    var srcFound = _findEntry(manifest, fname);
    if (!srcFound) throw new Error('file not found');
    if (_isProtected(srcFound.entry.name)) throw new Error('read only file');

    var destFound = _findEntry(manifest, dname);
    if (destFound) throw new Error('destination file already exists');
    ...
    if (res === 'Error: Disk Full') throw new Error('Disk Full');
