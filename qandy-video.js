...
function video_js() {
      ...
      switch (cmd) {
        case 'm': // SGR - set graphics rendition
          ...
          for (var pi = 0; pi < params.length; pi++) {
            ...
            else if (p === 38 && pi + 2 < params.length && params[pi + 1] === 5) { CURFG = 256 + params[pi + 2]; pi += 2; }
            else if (p === 39) { CURFG = 37; }
            else if (p === 48 && pi + 2 < params.length && params[pi + 1] === 5) { CURBG = 256 + params[pi + 2]; pi += 2; }
        ...
        case 'm': // SGR
          ...
          for (var pi = 0; pi < params.length; pi++) {
            ...
            switch (p) {
              ...
              case 38:
                if (pi + 2 < params.length && params[pi + 1] === 5) { CURFG = 256 + params[pi + 2]; pi += 2; }
                break;
              case 39: CURFG = 37; break;
              case 48:
                if (pi + 2 < params.length && params[pi + 1] === 5) { CURBG = 256 + params[pi + 2]; pi += 2; }
                break;
  ...
  window.pokeCell = pokeCell;
  window.pokeText = pokeText;
  window.pokeColor = pokeColor;
  window.pokeChar = pokeChar;
  window.pokeFG = pokeFG;
  window.pokeBG = pokeBG;
  window.pokeAttr = pokeAttr;
  window.pokeAttrBit = pokeAttrBit;
  window.pokeInverse = pokeInverse;
  window.pokeBold = pokeBold;
  window.pokeItalic = pokeItalic;
  window.pokeLine = pokeLine;
  window.pokeBlink = pokeBlink;
  window.pokeSelect = pokeSelect;
  window.pokeScroll = pokeScroll;
  window.pokeCursorOn = pokeCursorOn;
  window.pokeCursorOff = pokeCursorOff;
  window.pokeMenu = pokeMenu;
  window.pokeCursor = pokeCursor;
  window.pokeModem = pokeModem;
  window.pokeRefresh = pokeRefresh;
  window.pokeInput = pokeInput;
  window.peekChar = peekChar;
  window.peekFG = peekFG;
  window.peekBG = peekBG;
  window.peekAttr = peekAttr;
  window.peekBold = peekBold;
  window.peekItalic = peekItalic;
  window.peekBlink = peekBlink;
  window.peekLine = peekLine;
  window.peekInverse = peekInverse;
