...
// ── 1. Parse the item file path ───────────────────────────────────────────
  ...
  while (column < tape.length) {
    ...
    switch (word) {
      ...
      case 'Xi': { 
        ...
        if (!conditionMet) {
        	 // close the gate; let main loop handle the skip!
          gateopen = false;
        }
        break;
      }
      ...
      // ── Xd — Dice Roll / Random Number Generator ─────────────────────────────
      // Example: XdWd1020  -> Set Wd to random 10-20
      // Example: XdWdWs99  -> Set Wd to random (value of Ws) to 99
      case 'Xd': {
        var targetReg = tape.slice(column, column + 2); column += 2;
        var minArg    = tape.slice(column, column + 2); column += 2;
        var maxArg    = tape.slice(column, column + 2); column += 2;

        // Resolve min argument (Register or Literal)
        var minVal = 0;
        if (minArg.charAt(0) === 'W') {
          minVal = parseInt(register[minArg] || "0", 10);
        } else {
          minVal = parseInt(minArg, 10) || 0;
        }

        // Resolve max argument (Register or Literal)
        var maxVal = 0;
        if (maxArg.charAt(0) === 'W') {
          maxVal = parseInt(register[maxArg] || "0", 10);
        } else {
          maxVal = parseInt(maxArg, 10) || 0;
        }

        // Sanity check: if min is accidentally higher than max, swap them
        if (minVal > maxVal) {
          var temp = minVal;
          minVal = maxVal;
          maxVal = temp;
        ...
        // Roll the dice! (Inclusive of both min and max)
        var roll = Math.floor(Math.random() * (maxVal - minVal + 1)) + minVal;

        // Hard cap limits
        if (roll > 99) roll = 99;
        if (roll < 0) roll = 0;

        // Pad to two characters (e.g. 7 -> "07") and save
        var rollStr = (roll < 10 ? '0' : '') + roll;
        register[targetReg] = rollStr;

        break;
      }      
      
      default: {
        // ── DYNAMIC REGISTER COMMANDS (e.g., W1++05, Wa==W2) ──────────────
        if (word.charAt(0) === 'W') {
          var regTarget = word; // The register we are evaluating/modifying
          var op        = tape.slice(column, column + 2); column += 2; // Operator (++, --, etc)
          var operand   = tape.slice(column, column + 2); column += 2; // Value or Register (05, W2, etc)

          // 1. Resolve the operand value
          var applyValue = 0;
          if (operand.charAt(0) === 'W') {
            // It's another register! Grab its value
            applyValue = parseInt(register[operand] || "0", 10);
          } else {
            // It's a literal number (e.g. "05")
            applyValue = parseInt(operand, 10);
          }

          // 2. Get current value of target register
          var currentVal = parseInt(register[regTarget] || "0", 10);
          var conditionMet = true;

          // 3. Execute the Operator
          switch (op) {
            case '!!': // SET
              currentVal = applyValue;
              break;
            case '++': // ADD
              currentVal += applyValue;
              break;
            case '--': // SUBTRACT
              currentVal -= applyValue;
              if (currentVal < 0) currentVal = 0; // Prevent negative unless you want signed ints!
              break;
            case '==': // EQUALS
              conditionMet = (currentVal === applyValue);
              break;
            case '>>': // GREATER THAN
              conditionMet = (currentVal > applyValue);
              break;
            ...
            case '??': // DICE ROLL
              var operand2 = tape.slice(column, column + 2); column += 2;
              if (operand2.charAt(0) === 'W') {
                val2 = parseInt(register[operand2] || "0", 10);
              } else {
                val2 = parseInt(operand2, 10) || 0;
              }
              var minVal = val1;
              var maxVal = val2;
              // Swap if min is accidentally higher than max
              if (minVal > maxVal) { var temp = minVal; minVal = maxVal; maxVal = temp; }
              currentVal = Math.floor(Math.random() * (maxVal - minVal + 1)) + minVal;
              break;
            }
      
            default:
              console.log('UNIVAC: unknown register operator [' + op + '] at column ' + (column - 4));
              break;
          }

          // 4. Invert condition if 'Xn' (NOT) gate is open
          if (op === '==' || op === '>>' || op === '<<') {
            if (ifnot) conditionMet = !conditionMet;
            
            // If condition fails, SKIP ahead until we hit 'Xc' (Close condition)
            if (!conditionMet) {
              while (column < tape.length) {
          ...
          // 5. If it was a math operation, save the new value back to the register
          else if (op === '!!' || op === '++' || op === '--') {
            // Pad to 2 digits (e.g. 5 -> "05")
            var countStr = (currentVal < 10 ? '0' : '') + (currentVal > 99 ? 99 : currentVal);
            register[regTarget] = countStr;
            
            // Since a global config register was updated, save it to disk
            saveconfig(driveName); 
          }

          break; // End of dynamic register command
        }
  ...
  // calculate register values and update inventory string in state.content
