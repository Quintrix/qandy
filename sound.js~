...
// Replace existing beep(...) with this version (supports optional volume param)
// and add a volume(level) API to set/get global beep volume.

window.BEEP_VOLUME = (typeof window.BEEP_VOLUME !== 'undefined') ? window.BEEP_VOLUME : 0.3;

function beep(frequency = 800, duration = 200, volume) {
  try {
    ...
    // If caller did not provide volume, use global BEEP_VOLUME
    var vol = (typeof volume === 'number') ? volume : (window.BEEP_VOLUME || 0.3);
    vol = Math.max(0, Math.min(1, vol)); // clamp 0.0..1.0

    // Create oscillator (tone generator) and gain node
...
// volume(level)
// - If called with no args, returns current global beep volume (0.0..1.0).
// - If called with a numeric level, sets global default beep volume (clamped 0.0..1.0) and returns the previous value.
function volume(level) {
  if (typeof level === 'undefined') {
    return (typeof window.BEEP_VOLUME === 'number') ? window.BEEP_VOLUME : 0.3;
  }
