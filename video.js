...
window.pokeCursor = function(t) {
  ...
  // batch size for DOM refresh: update DOM after this many printable characters
  var BATCH_SIZE = 8;
  ...
  function processStep() {
    ...
    // Batch DOM refresh: accumulate up to BATCH_SIZE printable chars before refreshing
    var batchStartX = -1, batchStartY = -1, batchCount = 0;

    function flushBatch() {
      if (batchCount > 0) {
        pokeRefresh(batchStartX, batchStartY, batchCount);
        batchCount = 0;
        batchStartX = -1; batchStartY = -1;
      }
    }
    ...
    while (state.idx < state.str.length && state.charsEmitted < charsAllowed) {
      ...
      // Write directly to arrays (avoids per-cell DOM refresh via pokeCell/SYNC)
      if (CURX >= 0 && CURX < W && CURY >= 0 && CURY < H) {
        ...
        VIDEO[CURY][CURX] = ch;
        FCOLOR[CURY][CURX] = CURFG;
        BCOLOR[CURY][CURX] = CURBG;
        ATTR[CURY][CURX] = CURATTR;
        batchCount++;
      }
...
window.pokeScroll = function(n) {
  // Hide cursor while we mutate screen state
  // if (typeof pokeCursorOff === 'function') try { pokeCursorOff(); } catch (e) {}

  if (typeof n === 'undefined') n = 0;
  n = Number(n) || 0;
  if (n === 0) {
    // restore cursor and exit
    // if (typeof pokeCursorOn === 'function') try { pokeCursorOn(); } catch (e) {}
    return true;
  }

  // clamp distance to screen height
  var distance = Math.max(-H, Math.min(H, n));
  var defAttr = 0;

  // defaults for new/cleared cells
  var defaultFG = (typeof window.defaultColor !== 'undefined') ? window.defaultColor : ((window.currentStyle && currentStyle.color) ? currentStyle.color : 37);
  var defaultBG = (typeof window.defaultBg !== 'undefined') ? window.defaultBg : ((window.currentStyle && currentStyle.bgcolor) ? currentStyle.bgcolor : 40);

  // helpers to ensure typed arrays / arrays exist
  function ensureRowBanks(y) {
    if (!window.FCOLOR) FCOLOR = [];
    if (!window.BCOLOR) BCOLOR = [];
    if (!FCOLOR[y]) FCOLOR[y] = new Uint8Array(W);
    if (!BCOLOR[y]) BCOLOR[y] = new Uint8Array(W);
    if (!window.ATTR) ATTR = [];
    if (!ATTR[y]) {
      try { ATTR[y] = new Uint16Array(W); } catch (e) { ATTR[y] = new Array(W).fill(0); }
    }
    if (window.COLOR && !COLOR[y]) COLOR[y] = new Array(W);
  }

  // helper to set legacy COLOR object at (y,x)
  function setLegacyColor(y, x, fg, bg, attrObj) {
    ...
    o.bgcolor = bg;
    if (attrObj) {
      if (typeof attrObj.bold !== 'undefined') o.bold = !!attrObj.bold;
      if (typeof attrObj.inverse !== 'undefined') o.inverse = !!attrObj.inverse;
      if (typeof attrObj.italic !== 'undefined') o.italic = !!attrObj.italic;
      if (typeof attrObj.underline !== 'undefined') o.underline = !!attrObj.underline;
      if (typeof attrObj.dim !== 'undefined') o.dim = !!attrObj.dim;
      if (typeof attrObj.blink !== 'undefined') o.blink = !!attrObj.blink;
    }
    COLOR[y][x] = o;
  }
    ...
    for (var y = 0; y < H; y++) {
      ...
      if (src < H) {
        ...
        for (var x = 0; x < W; x++) {
          ...
          FCOLOR[y][x] = FCOLOR[src][x];
          BCOLOR[y][x] = BCOLOR[src][x];
          ATTR[y][x]   = (ATTR[src] && typeof ATTR[src][x] !== 'undefined') ? ATTR[src][x] : defAttr;

          // sync legacy color object if present
          if (window.COLOR) {
            var srcCol = (COLOR[src] && COLOR[src][x]) ? COLOR[src][x] : null;
            if (srcCol) {
              setLegacyColor(y, x, srcCol.color, srcCol.bgcolor, srcCol);
            } else {
              setLegacyColor(y, x, FCOLOR[y][x], BCOLOR[y][x], {});
            }
          }
        }
      } else {
        // fill bottom lines with blanks + defaults
        for (var x2 = 0; x2 < W; x2++) {
          VIDEO[y][x2] = ' ';
          FCOLOR[y][x2] = defaultFG;
          BCOLOR[y][x2] = defaultBG;
          ATTR[y][x2] = defAttr;
          if (window.COLOR) setLegacyColor(y, x2, defaultFG, defaultBG, {});
        }
      }

      // refresh the updated row at once
      if (typeof pokeRefreshRow === 'function') {
        try { pokeRefreshRow(0, y, W); } catch (e) { pokeRefresh(0, y, W); }
      } else {
        try { pokeRefresh(0, y, W); } catch (e) { /* best-effort */ }
      }
    }
  } else {
    ...
    for (var y2 = H - 1; y2 >= 0; y2--) {
      ...
      if (src2 >= 0) {
        ...
        for (var x3 = 0; x3 < W; x3++) {
          ...
          FCOLOR[y2][x3] = FCOLOR[src2][x3];
          BCOLOR[y2][x3] = BCOLOR[src2][x3];
          ATTR[y2][x3]   = (ATTR[src2] && typeof ATTR[src2][x3] !== 'undefined') ? ATTR[src2][x3] : defAttr;

          if (window.COLOR) {
            var sc = (COLOR[src2] && COLOR[src2][x3]) ? COLOR[src2][x3] : null;
            if (sc) setLegacyColor(y2, x3, sc.color, sc.bgcolor, sc);
            else setLegacyColor(y2, x3, FCOLOR[y2][x3], BCOLOR[y2][x3], {});
          }
        }
      } else {
        // fill top lines with blanks + defaults
        for (var x4 = 0; x4 < W; x4++) {
          VIDEO[y2][x4] = ' ';
          FCOLOR[y2][x4] = defaultFG;
          BCOLOR[y2][x4] = defaultBG;
          ATTR[y2][x4] = defAttr;
          if (window.COLOR) setLegacyColor(y2, x4, defaultFG, defaultBG, {});
        }
      }

      // refresh the updated row at once
      if (typeof pokeRefreshRow === 'function') {
        try { pokeRefreshRow(0, y2, W); } catch (e) { pokeRefresh(0, y2, W); }
      } else {
        try { pokeRefresh(0, y2, W); } catch (e) { /* best-effort */ }
      }
    }
  }

  // Adjust cursor and input-line anchors: distance > 0 means rows moved up => cursor row reduces
  function clampRow(r) {
