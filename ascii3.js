...
// - Produces a filtered string for the Qandy keyboard ranges: 1-31, 32-127, 168-223
...
function buildRangeString(start, end, useCp437) {
  ...
  for (var code = start; code <= end; code++) {
    var ch;
    if (useCp437 && CP437.hasOwnProperty(code)) {
      ch = CP437[code];
    } else {
...
// (you can narrow this if you want; here we show a full display-capable set)
...
// Step B: assemble "printable" Qandy characters (no numeric codes shown) as you requested
...
cls();
print("[-black][cls][bold][bgreen]  ╔══════════════════════════╗\n");
print("  ║   [yellow]QANDY CHARACTER MAP:[bgreen]   ║\n");
print("  ╚══════════════════════════╝\n\n[white] ");
