...
// ── 1. Parse the item file path ───────────────────────────────────────────
  ...
  while (column < tape.length) {
    ...
    switch (word) {
      ...
      default: {
        // REGISTER LOGIC PROCESSOR  
        // perform math/gate logic on the registers
        if (word.charAt(0) === 'W') {
          var op = tape.slice(column, column + 2); column += 2;
          var opStr1 = tape.slice(column, column + 2); column += 2;
          
          // GATE OPEN LOGIC: if the gate is closed, we break out NOW
          if (!gateopen) { break; }

          // Grab the current value of the target register
          var currentVal = parseInt(register[word] || "0", 10);

          // Helper to resolve an operand string (e.g. "05", "W2") to its integer value
          var val1 = 0;
          if (opStr1.charAt(0) === 'W') {
            val1 = parseInt(register[opStr1] || "0", 10);
          } else {
            val1 = parseInt(opStr1, 10) || 0;
          }
          ...
          switch (op) {
            case '!!': // SET (Implied explicit set)
              register[word] = val1.toString().padStart(2, '0');
              break;

            case '++': // ADD
              var added = currentVal + val1;
              if (added > 99) added = 99; // hard limit
              register[word] = added.toString().padStart(2, '0');
              break;

            case '--': // SUBTRACT
              var subbed = currentVal - val1;
              if (subbed < 0) subbed = 0; // hard limit
              register[word] = subbed.toString().padStart(2, '0');
              break;

            case '??': // DICE ROLL
              var roll = Math.floor(Math.random() * (val1)) + currentVal;
              register[word] = roll.toString().padStart(2, '0');
              break;

            // --- Logic Gates ---
            case '==': // EQUALS
              conditionMet = (currentVal === val1);
              if (ifnot) conditionMet = !conditionMet; // apply the Xn inversion if active
              if (!conditionMet) gateopen = false;     // close the gate
              break;

            case '>>': // GREATER THAN
              conditionMet = (currentVal > val1);
              if (ifnot) conditionMet = !conditionMet;
              if (!conditionMet) gateopen = false;
              break;
            ...
            default:
              console.log('UNIVAC: unknown register operator [' + op + '] at column ' + (column - (isDiceRoll ? 6 : 4)));
              break;
          }
